...
modified_df = df[['temp', 'prcp', 'dwpt', 'rhum', 'wspd', 'pres']].copy()
...
def create_sequences(data, input_len=72, output_len=6):
    X, y = [], []
...
X, y = create_sequences(scaled_data, 72, 6)
...
model = Sequential([
    Bidirectional(LSTM(64, return_sequences=True, activation='tanh'), input_shape=(12, X.shape[2])),
    Dropout(0.3),
...
#Predict using the last 72 hours from test set
...
pred_inv[:, :, 3] = np.expm1(pred_inv[:, :, 3])
actual_inv[:, :, 3] = np.expm1(actual_inv[:, :, 3])
...
plt.title('Temperature (°C) - Next 6 Hours')
plt.xlabel('Hours Ahead')
plt.ylabel('Temperature')
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(hours, actual_inv[sample_idx, :, 3], label='Actual Precipitation')
plt.plot(hours, pred_inv[sample_idx, :, 3], label='Predicted Precipitation')
plt.title('Precipitation (mm) - Next 6 Hours')
plt.xlabel('Hours Ahead')
